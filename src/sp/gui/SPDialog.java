...
public class SPDialog extends Dialog
{
	...
	private void createContents() {
	    ...
	    tabMain.setText("РќР°СЃС‚СЂРѕР№РєРё");
		...
		/*Listener paintListener = new Listener() {
		      public void handleEvent(Event event) {
		        ...
		        case SWT.MeasureItem: {
		          ...
		          if(text.startsWith("РЈСЃС‚Р°РЅР°")){
		...
		btnOk.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				while(iterator.hasNext()){
					...
					blockItem = new TableItem(table, SWT.NONE);
					if(!block.getBlockTitle().equals("Р”РѕРєСѓРјРµРЅС‚Р°С†РёСЏ")){
					...
					if(block.getBlockType().equals("Default") && iterator.nextIndex()!=blockList.size()){
						if(blockList.get(iterator.nextIndex()).getBlockType().equals("ME")){
							blockItem = new TableItem(table, SWT.NONE);
							blockItem.setText(new String[]{"РЈСЃС‚Р°РЅР°РІР»РёРІР°РµС‚СЃСЏ РїРѕ " + Specification.settings.getStringProperty("MEDocumentId")});
